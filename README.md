...
> **项目状态：工程骨架阶段。** 已建立 Vue 前端、Python FastAPI 后端与健康检查；本文的业务功能仍为规划，资料导入、问答、数据库与学习计划尚未实现。

## 开发与架构入口

- [技术选型、目录分层与架构图](docs/ARCHITECTURE.md)
- [本地启动与验证命令](docs/DEVELOPMENT.md)
- [贡献指南](CONTRIBUTING.md)

工程采用前后端分离的模块化单体：Vue 3 + TypeScript + Vite，Python 3.12 + FastAPI。持久化计划采用 PostgreSQL + pgvector，耗时解析计划采用 RQ + Redis 和独立 Python Worker；这些基础设施将在相应功能开发时接入。
...
已建立工程骨架与架构基线，后续业务实现按以下顺序推进：

1. 收集经过授权和必要脱敏的课程资料样本，建立代表性场景与验收集。
2. 设计“导入—核对—问答—计划”的页面原型，确认字段、状态与交互。
3. 定义业务数据模型、检索引用规范、计划约束和操作确认协议。
4. 按架构基线接入持久化、解析、检索和作业队列，评估具体模型服务，验证端到端最小流程。
5. 根据验收结果迭代，再扩展输入格式、通知变更、课程路径与外部服务。

## 11. 参与贡献与许可证

需求反馈、文档完善、样本整理和代码贡献请参阅 [贡献指南](CONTRIBUTING.md)。项目背景与业务约定见 [AGENTS.md](AGENTS.md)。

**许可证待定。** 小组尚未选择项目许可证，当前未提供 `LICENSE`，不声明采用开源许可证；确定后将在此处说明授权方式与适用范围。
